...
List<String> generateBuildings() {
  ...
  for (int b = 0; b < 4; b++) {
    int x0 = rng.nextInt(150);
    int y0 = rng.nextInt(200);
    int w = rng.nextInt(20) + 10;
    int h = rng.nextInt(20) + 10;
    double roofZ = rnd(5, 15);
    ...
    for (int z = 0; z < roofZ.toInt(); z += 2) {
      for (int x = 0; x < w; x++) {
        out.add(lidarPoint(x0 + x, y0, z, rnd(60, 110)));
        out.add(lidarPoint(x0 + x, y0 + h, z, rnd(60, 110)));
      }
      for (int y = 0; y < h; y++) {
        out.add(lidarPoint(x0, y0 + y, z, rnd(60, 110)));
        out.add(lidarPoint(x0 + w, y0 + y, z, rnd(60, 110)));
      }
    }
  }
